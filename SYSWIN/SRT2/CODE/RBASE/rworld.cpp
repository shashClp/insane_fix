...
#include "RWORLD.H"
//±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
byte *VAR_VIDEO_VGA2 = (byte *) 0xA0000;
//±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
TVertex TDir;
float TScale = 10.0;
float TInvScale = 1.0/10.0;


float TNumMax=16.0;
float TSlat=0.6;
ProceduralTexture *TXT;
//±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
void TWorld_CalcLight (rgbcol &LightIntentsPNG,RayInfo &RayPtr,long &ID,TVertex &Dir,TVertex &Norm,TVertex &Check,long Fact)
{
  ...
  } else
  {
    ...
    TReal AMBIENT = (float)(256-ActiveWorld->AMBIENT)/256.0;
    for (register long TTL = 0; TTL < ActiveWorld->Num_Of_Lights; TTL++)
    {
      ...
      if ((!Shadow))
      {
        ...
        R += (AMBIENT)*RayPtr.LigthIntens*ActiveLight->Color.B.R;
        G += (AMBIENT)*RayPtr.LigthIntens*ActiveLight->Color.B.G;
        B += (AMBIENT)*RayPtr.LigthIntens*ActiveLight->Color.B.B;
...
*/
        *(dst+x) = RayPtr.Color.ColRGB;
				*(dst+x-1) = RayPtr.Color.ColRGB;
				*(dst+x+1) = RayPtr.Color.ColRGB;
				switch (FallasValue)
				{
					case 0 :
							*(dst+x-319) = RayPtr.Color.ColRGB;
							*(dst+x+319) = RayPtr.Color.ColRGB;
							break;
					case 1 :
							*(dst+x-320) = RayPtr.Color.ColRGB;
							*(dst+x+320) = RayPtr.Color.ColRGB;
							break;
					case 2 :
							*(dst+x-321) = RayPtr.Color.ColRGB;
							*(dst+x+321) = RayPtr.Color.ColRGB;
							break;
				}
      } else
...
void Ray_GetColorInfo2 ( RayInfo &RayPtr,TVertex &CH2){

TVertex CH = CH2;
TReal JumpX = TSlat*TDir.x;
TReal JumpY = TSlat*TDir.y;
TReal JumpZ = TSlat*TDir.z;
TReal JX = TTMin*TDir.x;
TReal JY = TTMin*TDir.y;
TReal JZ = TTMin*TDir.z;
...
P1.RGBA[0]  = RayPtr.Color.B.R;
P1.RGBA[1]  = RayPtr.Color.B.G;
P1.RGBA[2]  = RayPtr.Color.B.B;

while (Jump <TNumMax)
{
  RVTX_Set (CH,TPos.x + JX,TPos.y + JY,TPos.z + JZ);

  Col.RGBA[0] = Col.RGBA[1] = Col.RGBA[2] = Col.RGBA[3] = 0;

  GetTextureColor(TXT,CH,TScale,TInvScale,Col);
  P1.RGBA[0] = Col.RGBA[0]*P1.RGBA[3] + P1.RGBA[0]*(1-P1.RGBA[3]);
  P1.RGBA[1] = Col.RGBA[1]*P1.RGBA[3] + P1.RGBA[1]*(1-P1.RGBA[3]);
  P1.RGBA[2] = Col.RGBA[2]*P1.RGBA[3] + P1.RGBA[2]*(1-P1.RGBA[3]);
